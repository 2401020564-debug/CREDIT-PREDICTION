...
if st.button("Predict Default Risk"):
    ...
    input_data = pd.DataFrame([[
        ...
        open_credit_lines, time_90_days_late, real_estate_loans, import streamlit as st
import pandas as pd
import joblib
...
if st.button("Predict Default Risk"):
    ...
    # Make sure to change 0.26 if your radar script found a different number!
    ...
    # 1. Get the exact probability
    probability = model.predict_proba(input_data)[0][1]

    # 2. Apply your winning Sweet Spot threshold!
    # CHANGE THIS NUMBER to your actual optimal threshold
    optimal_threshold = 0.26 

    # 3. Show the result using the new smart logic!
    st.markdown("---")
    if probability >= optimal_threshold:
        st.error(f"⚠️ High Risk! The model predicts a Default. (Probability: {probability:.2%})")
    else:
